...
// 1.2. Note que uma das caixas está um pouco acima das outras. Por que isso ocorre? "Pq está com a classe tech"

// 2. Crie uma função que adicione a classe 'tech' ao elemento `li` quando for clicado.
// 2.1. Deve existir apenas um elemento com a classe 'tech'. Como você faz isso?

function adicionaClasse (event) {
    const comClasse = document.querySelector('.tech');
    comClasse.classList.remove('tech');
...
function mudaTexto (event) {
    let texto = document.querySelector('.tech').innerText;
    texto = input.value;
}
